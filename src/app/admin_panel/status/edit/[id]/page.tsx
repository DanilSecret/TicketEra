...
import { GetStatusById, UpdateStatus } from "@/app/Api/Api";
...
const validationSchema = Yup.object().shape({
    name: Yup.string()
        .min(2, 'Название статуса должно быть не короче 2 символов')
        .required('Название обязательно'),
    color: Yup.string()
        .matches(/^#([0-9a-f]{3}){1,2}$/i, 'Цвет должен быть в формате HEX (#RRGGBB)')
...
export default function EditStatusPage() {
    ...
    const { register, handleSubmit, formState: { errors }, reset } = useForm<StatusEInterface>({
        ...
        if (!cookies.auth_token || userData === null) {
            ...
            const fetchStatus = async () => {
                ...
                if (response.success) {
                    ...
                    setMessage(response.message || "Ошибка при загрузке статуса");
                }
            };
            fetchStatus();
        }
    ...
    const onSubmit = async (data: StatusEInterface) => {
        try {
            const response = await UpdateStatus(id, data.name, data.color, data.visible, cookies.auth_token);

            if (response.success) {
                router.push("/admin_panel/status/");
            } else {
                setMessage(response.message || "Ошибка обновления статуса");
            }
        } catch (error) {
            setMessage(`${error}`);
        }
        ...
        <div className="h-screen flex flex-col">
            ...
            <div className="flex items-center justify-center h-screen bg-gray-100 text-black">
                <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-sm bg-white p-6 rounded-lg shadow-md">
                    <h1 className="text-xl font-semibold text-center mb-4">Редактирование статуса</h1>
                    ...
                    <div className="mb-4">
                        <label className="block text-sm font-medium text-gray-700">Видимость</label>
                        <select
                            {...register("visible")}
                            className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                        >
