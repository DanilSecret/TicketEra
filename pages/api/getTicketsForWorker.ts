...
export default async function getTicketsForWorker(req: NextApiRequest, res: NextApiResponse) {
    if (req.method !== "GET") {
    ...
    try {
        ...
        let ticketsQuery = `
            SELECT tickets.*, status.name AS status_name, status.color AS status_color
            FROM tickets
