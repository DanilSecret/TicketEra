...
export default async function updateStatusByAdmin(req: NextApiRequest, res: NextApiResponse) {
    if (req.method !== "PUT") {
    ...
    try {
        ...
        const { id, name, color, visible } = req.body as {
            id: number;
            name: string;
            color: string;
            visible: boolean;
        ...
        if (!id || !name || !color || typeof visible !== "boolean") {
            return res.status(400).json({ message: "Поля заполнены некорректно" });
        }

        const result = await pool.query(
            "UPDATE status SET name = $1, color = $2, visible = $3 WHERE id = $4 RETURNING *",
            [name, color, visible, id]
        );
